...
import com.interview.customer_management.customer.rest.validation.CustomerValidationErrorCode;
import com.interview.customer_management.customer.rest.validation.CustomerValidationErrorResponse;
...
public class CustomerRestExceptionHandler {
  ...
  @ExceptionHandler
  public ResponseEntity<CustomerValidationErrorResponse> handleException(
      final SalesTaxIdValidationException ex) {
    LOGGER.warn("Error occurred when validating CustomerDto Sales tax id on REST API", ex);

    final var errorResponse =
        new CustomerValidationErrorResponse(
            CustomerValidationErrorCode.SALES_TAX_ID_VALIDATION_ERROR, System.currentTimeMillis());
  ...
  public ResponseEntity<CustomerValidationErrorResponse> handleException(
    ...
    final var errorResponse =
        new CustomerValidationErrorResponse(
            CustomerValidationErrorCode.BASIC_VALIADATION_ERROR, System.currentTimeMillis());

    return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
  }

  @ExceptionHandler
  public ResponseEntity<String> handleException(final CustomerNotFoundException ex) {
